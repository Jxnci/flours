  ...
  <div class="flex flex-col items-center justify-center w-full mx-auto">
    ...
    <div v-else class="w-full">
      <ul class="flex flex-col w-full">
        <li
          class="flex flex-row mb-2 border-gray-400"
          v-for="producto in productos"
          :key="producto.id"
        >
          <router-link
            :to="{ name: 'productoDetail', params: { id: producto.id } }"
          ...
          >
            ...
            <div class="flex-1 pl-1 md:mr-16">
              <div class="font-medium dark:text-white">{{ producto.name }}</div>
              <div class="text-sm text-gray-600 dark:text-gray-200">
                Categoria_Id:{{ producto.category_id }}
              </div>
            </div>
            <div class="text-xs text-gray-600 dark:text-gray-200">
              stock: {{ producto.stock }}(falta:S/.)
            </div>
