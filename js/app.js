...
class Presupuesto {
  ...
  calcularRestante() {
    ...
    let totalCantidad = this.gasto.reduce(
...
class UI {
  ...
  mostrarGasto(gastos) {
    ...
    gastos.forEach((gasto) => {
      ...
      li.className =
        "list-group-item d-flex justify-content-between align-items-center mt-3 p-2 bg-info";
  ...
  actualizarRestante(restante) {
    ...
    if (Number(restaPintar.textContent) <= presupuesto * 0.2) {
      ...
      restaPintar.parentElement.parentElement.classList.remove(
        "alert-danger",
        "alert-warning"
      );
